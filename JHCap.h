...
// Copyright 2011-2019 (C) Shenzhen Jinghang Technology Co., Ltd.
...
#define CAMERA_ERROR_OUT_OF_MEMORY           18
#define CAMERA_ERROR_RESET                   19

#define CAMERA_IMAGE_RAW   0x1
...
#define CAMERA_RESOLUTION_LINESCAN 0x3

#define CAMERA_LINE_TRIGGER_AUTO 0x00
#define CAMERA_LINE_TRIGGER_EXTERNAL 0x01
...
/*==============================================================
Name: CameraGetLastErro
Description: Get the last erro
...
/*==============================================================
Name: CameraInitIO
Description: Initialize the camera for get/set io, trigger.
Parameters: index, the index of camera to initialize
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: call CameraFree to release devic.
 --------------------------------------------------------------*/

extern "C" DLL_EXPORT API_STATUS __stdcall CameraInitIO(int device_id);
...
Return: returns API_OK on success, otherwise returns API_ERROR.
Note:  These properties can be set by usbVideo or programmingï¼?
       "image_format", 
	   "resolution_index"
	   "enable_roi"
	   "roi_offset_x"
 ...
 --------------------------------------------------------------*/
extern "C" DLL_EXPORT API_STATUS __stdcall CameraSetProperty(int device_id, char *name, int value);
...
Note: Refer to note for CameraSetProperty
 --------------------------------------------------------------*/
extern "C" DLL_EXPORT API_STATUS __stdcall CameraGetProperty(int device_id, char *name, int *value);
...
Parameters: device_id, the camera index
            *advanced_gain, return the value of advanced gain
Return: returns API_OK on success, otherwise returns API_ERROR.
Note:Range from 0-3
--------------------------------------------------------------*/
...
/*==============================================================
Name: CameraGetImageDepth
Description: Get the image's pixel bit depth.
Parameters: device_id, the camera index
            *depth, bits per pixel.
Return: returns API_OK on success, otherwise returns API_ERROR.
--------------------------------------------------------------*/

extern "C" DLL_EXPORT API_STATUS __stdcall CameraGetImageDepth(int device_id,int *depth);
...
Parameters: device_id, the camera index
            *lut, LUT data, a map of RAW12 data to RAW8 data.
            len, the length of the pre allocated lut
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: LUT only make sense for RAW12 output camera, the RAW12 data is converted to RAW8 by the LUT before further processing to display.
--------------------------------------------------------------*/
extern "C" DLL_EXPORT API_STATUS __stdcall CameraSetLUT(int device_id, unsigned char *lut, int len);

/*==============================================================
Name: CameraGetLUT
Description: Soft isp to covert raw data to color/gray image.
Parameters: device_id, the camera index
            *lut, return the LUT data in a pre-located array.
            *len, the length of the pre allocated lut. if lut is NULL, the len will return the length needed.
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: LUT only make sense for RAW12 output camera, the RAW12 data is converted to RAW8 by the LUT before further processing to display.
--------------------------------------------------------------*/
extern "C" DLL_EXPORT API_STATUS __stdcall CameraGetLUT(int device_id, unsigned char *lut, int *len);
...
/*==============================================================
Name: CameraGetSnapNum
Description: Get number of frames by one trigger.
Parameters: device_id, the camera index
...
extern "C" DLL_EXPORT API_STATUS __stdcall CameraGetSnapNum(int device_id, int *num);

/*==============================================================
Name: CameraSetSnapNum
Description: Set number of frames by one trigger.
Parameters: device_id, the camera index
...
extern "C" DLL_EXPORT API_STATUS __stdcall CameraSetSnapNum(int device_id, int num);
...
/*==============================================================
Name: CameraGetFrameRate
Description: Check if fps constrain enabled, and get the constrain fps.
Parameters: device_id, the camera index
            *en, return fps constrain enable status
			*fps return the fps expected.
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: 1)Only few models support this feature.
      2)The max fps is limited by the sensor output.
	  3)Set en to false will return to the max fps.
--------------------------------------------------------------*/

extern "C" DLL_EXPORT API_STATUS __stdcall CameraGetFrameRate(int device_id, bool *en, double *fps);

/*==============================================================
Name: CameraSetFrameRate
Description: Set fps constrain status, and set constrain fps.
Parameters: device_id, the camera index
            en, Enable or disable fps constrain
			fps, fps expected.
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: 1)Only few models support this feature.
      2)The max fps is limited by the sensor output.
...
extern "C" DLL_EXPORT API_STATUS __stdcall CameraSetFrameRate(int device_id, bool en, double fps);
...
/*==============================================================
Name: CameraSetDenoise
Description: Enable or disable denoise function
Parameters: device_id, the camera index
            en, true for enable and false for disable
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: Only valid for mono camera with GRAY8 mode.
--------------------------------------------------------------*/

extern "C" DLL_EXPORT API_STATUS __stdcall CameraSetDenoise(int device_id,bool en);

/*==============================================================
Name: CameraGetDenoise
Description: Get the denoise fuction status
Parameters: device_id, the camera index
            *en, return the state of denoise function, true for enabled and false for disabled. 
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: 
--------------------------------------------------------------*/

extern "C" DLL_EXPORT API_STATUS __stdcall CameraGetDenoise(int device_id,bool *en);

/*==============================================================
Name: CameraSetFPN
Description: Enable or disable FPN function
Parameters: device_id, the camera index
            en, true for enable and false for disable
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: Only valid for mono camera with GRAY8 mode.
...
extern "C" DLL_EXPORT API_STATUS __stdcall CameraSetFPN(int device_id,bool en);

/*==============================================================
Name: CameraGetFPN
Description: Get the FPN fuction status
Parameters: device_id, the camera index
            *en, return the state of FPN function, true for enabled and false for disabled. 
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: 
--------------------------------------------------------------*/

extern "C" DLL_EXPORT API_STATUS __stdcall CameraGetFPN(int device_id,bool *en);
...
/*==============================================================
Name: CameraSetLineHeight
Description: Set line scane line height
Parameters: device_id, the camera index
            line_height, lines, 2, 4, 6, 8, etc with area scan sensor.
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: 
--------------------------------------------------------------*/

extern "C" DLL_EXPORT API_STATUS __stdcall CameraSetLineHeight(int device_id, int line_height);


/*==============================================================
Name: CameraGetLineHeight
Description: Get line scane line height
Parameters: device_id, the camera index
            *line_height, return scan lines count with area scan sensor.
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: 
--------------------------------------------------------------*/

extern "C" DLL_EXPORT API_STATUS __stdcall CameraGetLineHeight(int device_id, int *line_height);

/*==============================================================
Name: CameraSetLineTrigger
Description: Set line scane trigger mode
Parameters: device_id, the camera index
            line_trigger, two line trigger modes.
			              CAMERA_LINE_TRIGGER_AUTO  trigger by camera with maximum speed
			              CAMERA_LINE_TRIGGER_EXTERNAL  by camera external trigger signal
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: 
--------------------------------------------------------------*/

extern "C" DLL_EXPORT API_STATUS __stdcall CameraSetLineTrigger(int device_id, int line_trigger);

/*==============================================================
Name: CameraGetLineTrigger
Description: Get line scane trigger mode
Parameters: device_id, the camera index
            *line_trigger, return the line trigger mode.
			              CAMERA_LINE_TRIGGER_AUTO  trigger by camera with maximum speed
			              CAMERA_LINE_TRIGGER_EXTERNAL  by camera external trigger signal
Return: returns API_OK on success, otherwise returns API_ERROR.
Note: 
--------------------------------------------------------------*/
extern "C" DLL_EXPORT API_STATUS __stdcall CameraGetLineTrigger(int device_id, unsigned char *line_trigger);
